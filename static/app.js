...
function convertQuantity(quantity, fromUnit, toUnit) {
  const [fromGroup, fromFactor] = UNIT_FACTORS[fromUnit];
  const [toGroup, toFactor] = UNIT_FACTORS[toUnit];
  if (fromGroup !== toGroup) throw new Error(`Cannot convert ${fromUnit} to ${toUnit}`);
  return (quantity * fromFactor) / toFactor;
}
...
recipeForm.addEventListener('submit', async (e) => {
  ...
  if (!resp.ok) {
    const text = await resp.text();
    document.getElementById('results').textContent = `Unable to save recipe: ${text}`;
    return;
  }
